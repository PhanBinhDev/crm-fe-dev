...
export const useActivityActions = () => {
  ...
  const markComplete = useCallback(async (
    ...
    try {
      ...
      const completedStage = sortedStages.find(stage => 
      ...
      // API call để cập nhật cả stageId và status cùng lúc
      updateActivity({
        resource: 'activities',
        id: activity.id,
        values: { 
  ...
  const duplicateActivity = useCallback(async (activity: IActivity) => {
    try {
      ...
      const duplicateData: FormAddActivityPayload = {
        ...
        assignees: assignees?.map(assignee => assignee.user.id) || [],
        files: undefined, // Don't duplicate files
      };
  ...
  const removeActivity = useCallback(async (activity: IActivity) => {
    deleteActivity({
      resource: 'activities',
      id: activity.id
    }, {
  ...
  const createSubtask = useCallback(async (activity: IActivity) => {
    try {
